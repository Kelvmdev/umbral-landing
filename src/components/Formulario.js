...
const inputBase =
  "w-full rounded-xl border border-linea bg-papel px-4 py-3 font-body text-tinta outline-none focus:border-arcilla";
...
export default function Formulario() {
  ...
  return (
    <form
      onSubmit={enviar}
      noValidate
      className="mx-auto w-full max-w-xl rounded-[1.5rem] border border-linea bg-papel-2 p-6 text-left sm:p-8"
    >
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block">
          <span className="font-body text-sm font-medium text-tinta-suave">Nombre</span>
          <input name="nombre" type="text" required minLength={2} maxLength={80} autoComplete="name" className={`mt-1 ${inputBase}`} />
        </label>
        <label className="block">
          <span className="font-body text-sm font-medium text-tinta-suave">Celular</span>
          <input name="celular" type="tel" required inputMode="numeric" pattern="[0-9+\s\-]{7,20}" autoComplete="tel" className={`mt-1 ${inputBase}`} />
        </label>
      </div>

      <label className="mt-4 block">
        <span className="font-body text-sm font-medium text-tinta-suave">Correo</span>
        <input name="correo" type="email" required maxLength={120} autoComplete="email" className={`mt-1 ${inputBase}`} />
      </label>

      <label className="mt-4 block">
        <span className="font-body text-sm font-medium text-tinta-suave">¿Qué buscas?</span>
      ...
      <label className="mt-4 block">
        <span className="font-body text-sm font-medium text-tinta-suave">Mensaje (opcional)</span>
        <textarea name="mensaje" rows={3} maxLength={1000} className={`mt-1 ${inputBase}`} />
      </label>
      ...
      {estado.msg && (
        <p
          role="status"
          className={`mt-4 font-body text-sm ${estado.tipo === "error" ? "text-arcilla" : "text-oliva"}`}
        >
          {estado.msg}
        </p>
      )}
      ...
      <button
        ...
        className={`${btnArcilla} mt-6 w-full disabled:opacity-60 disabled:active:scale-100`}
      >
